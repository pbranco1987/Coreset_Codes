...
Every theoretical construct in this document corresponds to one or more specific Python symbols. Use this table to jump from equations and algorithms to code. For detailed signatures and examples of each symbol, see the [API Reference](./api/index.md).

### Objective Functions

| Concept | File | Symbol |
|---------|------|--------|
| Symmetric KL divergence between diagonal Gaussians | `coreset_selection/objectives/skl.py` | `symmetric_kl_diag_gaussians` |
| Forward / reverse KL | `coreset_selection/objectives/skl.py` | `kl_diag_gaussians`, `jeffreys_divergence_diag_gaussians` |
| MMD² proxy via Random Fourier Features | `coreset_selection/objectives/mmd.py` | `RFFMMD`, `compute_rff_features`, `mmd2_exact` |
| Sinkhorn divergence (anchor-based) | `coreset_selection/objectives/sinkhorn.py` | `AnchorSinkhorn`, `sinkhorn2_safe` |
| Nyström log-det diversity | `coreset_selection/objectives/nystrom_logdet.py` | `NystromLogDet` |
| Unified multi-objective computer | `coreset_selection/objectives/computer.py` | `SpaceObjectiveComputer`, `build_space_objective_computer` |

### Geographic Constraints

| Concept | File | Symbol |
|---------|------|--------|
| GeoInfo dataclass (group structure, targets π and π_pop) | `coreset_selection/geo/info.py` | `GeoInfo`, `build_geo_info` |
| KL divergence `D_α(π, π̂)` | `coreset_selection/geo/kl.py` | `kl_pi_hat_from_counts`, `kl_weighted_from_subset` |
| **Algorithm 2 (KL-optimal integer quota)** | `coreset_selection/geo/kl.py` | `kl_optimal_integer_counts_bounded` |
| **KL_min(k) analytical floor** (Theorem 1) | `coreset_selection/geo/kl.py` | `min_achievable_geo_kl_bounded` |
| Quota path `c*(k), KL_min(k)` over grid | `coreset_selection/geo/kl.py` | `compute_quota_path`, `save_quota_path` |
| Hard-quota projection | `coreset_selection/geo/projector.py` | `GeographicConstraintProjector.project_to_quota_mask` |
| Exact-k cardinality projection | `coreset_selection/geo/projector.py` | `project_to_exact_k_mask` |
| Soft KL constraint (population-share) | `coreset_selection/constraints/proportionality.py` | `build_population_share_constraint`, `ProportionalityConstraint` |
| Soft KL constraint (municipality-share) | `coreset_selection/constraints/proportionality.py` | `build_municipality_share_constraint` |
| **Algorithm 1 (swap-based repair)** | `coreset_selection/constraints/proportionality.py` | `ProportionalityConstraintSet.repair` |
| `τ` calibration utilities (Phase 5) | `coreset_selection/constraints/calibration.py` | `estimate_feasible_tau_range`, `tau_sensitivity_sweep` |

### Optimisation

| Concept | File | Symbol |
|---------|------|--------|
| NSGA-II main loop | `coreset_selection/optimization/nsga2_internal.py` | `nsga2_optimize` |
| Deb's non-dominated sort | `coreset_selection/optimization/nsga2_internal.py` | `fast_non_dominated_sort` |
| Constraint-dominated sort (Deb's constrained-domination) | `coreset_selection/optimization/nsga2_internal.py` | `_constraint_dominated_sort` |
| Crowding distance | `coreset_selection/optimization/selection.py` | `crowding_distance` |
| Greedy KL-floor initialisation (Corollary 2) | `coreset_selection/optimization/nsga2_internal.py` | `_greedy_kl_init` |
| Repair operator wrapper (calls Algorithm 1) | `coreset_selection/optimization/_nsga2_operators.py` | `_repair_mask` |
| Knee selection (L2 distance to utopia) | `coreset_selection/optimization/selection.py` | `select_knee` |
| Full representative selection (knee + best-per-obj) | `coreset_selection/optimization/selection.py` | `select_pareto_representatives` |
| **Adaptive τ state machine** (probe / bisect / production) | `scripts/launchers/adaptive_tau.py` | `run_adaptive_tau_nsga2` |

### Evaluation

| Concept | File | Symbol |
|---------|------|--------|
| Nyström approximation error `‖K_EE − K̂‖_F / ‖K_EE‖_F` | `coreset_selection/evaluation/raw_space.py` | `RawSpaceEvaluator.nystrom_error` |
| Kernel-PCA distortion | `coreset_selection/evaluation/raw_space.py` | `RawSpaceEvaluator.kpca_distortion` |
| KRR RMSE on Nyström features | `coreset_selection/evaluation/raw_space.py` | `RawSpaceEvaluator.krr_rmse` |
| **S ∩ E overlap exclusion** (landmarks ≠ evaluation points) | `coreset_selection/evaluation/raw_space.py` | `RawSpaceEvaluator._get_nystrom` |
| Multi-model downstream (KNN, RF, LR, GBT on Nyström features) | `coreset_selection/evaluation/raw_space.py` | `RawSpaceEvaluator.multi_model_downstream` |
| Geographic diagnostics (KL, L1, MaxDev) for both weight modes | `coreset_selection/evaluation/geo_diagnostics.py` | `dual_geo_diagnostics` |
| State-conditioned KPI stability (manuscript Section VII) | `coreset_selection/evaluation/kpi_stability.py` | `state_kpi_stability`, `state_krr_stability` |
| R11 proxy-stability diagnostics | `coreset_selection/evaluation/r7_diagnostics.py` | `run_r7_diagnostics`, `surrogate_sensitivity_analysis` |
| Cross-method championship (Friedman, Nemenyi, Cliff's δ) | `coreset_selection/evaluation/method_comparison.py` | `effect_isolation_table`, `rank_table`, `build_comparison_report` |

### Data & Representation Learning

| Concept | File | Symbol |
|---------|------|--------|
| Mixed-type preprocessing pipeline | `coreset_selection/data/preprocessing.py` | `impute_missing_values`, `_br_to_float`, `detect_numeric_columns` |
| Tabular VAE model | `coreset_selection/models/vae.py` | `TabularVAE`, `VAETrainer` |
| PCA fitting and projection | `coreset_selection/models/pca.py` | `fit_pca`, `pca_embed` |
| Replicate cache builder (VAE + PCA + splits + targets) | `coreset_selection/data/cache.py` | `build_replicate_cache` |
| Replicate cache loader | `coreset_selection/data/cache.py` | `load_replicate_cache` |
| **Target-leakage detection & removal (Phase 4.3)** | `coreset_selection/data/target_columns.py` | `detect_target_columns`, `remove_target_columns`, `validate_no_leakage`, `TARGET_COLUMN_PATTERNS` |
| Stratified split persistence | `coreset_selection/data/split_persistence.py` | `save_splits`, `load_splits` |

### Baselines (Manuscript Table VI)

Each baseline has two variants: unconstrained and quota-constrained. All live in `coreset_selection/baselines/`.

| Method | Unconstrained | Quota-constrained |
|--------|---------------|-------------------|
| Uniform random | `baseline_uniform` | `baseline_uniform_quota` |
| k-Means representatives | `baseline_kmeans_reps` | `baseline_kmeans_reps_quota` |
| Kernel Herding | `baseline_kernel_herding` | `baseline_kernel_herding_quota` |
| Farthest-First (k-Center) | `baseline_farthest_first` | `baseline_farthest_first_quota` |
| Ridge Leverage Score | `baseline_rls` | `baseline_rls_quota` |
| k-DPP | `baseline_dpp` | `baseline_dpp_quota` |
| Kernel Thinning | `baseline_kernel_thinning` | `baseline_kernel_thinning_quota` |
| Kernel k-Means (Nyström) | `baseline_kkmeans_nystrom` | `baseline_kkmeans_nystrom_quota` |

### Scripts (CLI Entry Points)

| Script | Role |
|--------|------|
| `scripts/launchers/build_caches.py` | Build replicate caches from raw CSVs. |
| `scripts/launchers/adaptive_tau.py` | Primary NSGA-II launcher with 3-phase τ calibration. |
| `scripts/launchers/run_baselines.py` | Run & soft-KL-repair the 8 baselines. |
| `scripts/analysis/evaluate_coresets.py` | Standalone metric evaluation (5-stage pipeline). |
| `scripts/analysis/championship.py` | Friedman / Nemenyi / head-to-head ranking. |
| `scripts/bootstrap_reeval.py` | Bootstrap target re-evaluation for robustness. |
