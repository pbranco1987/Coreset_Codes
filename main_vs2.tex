...
reporting periods. Similar scaling pressures appear well beyond telecom, motivating approximation-based kernel
...
Landmark selection is performed once per cross-section and configuration, enabling repeated downstream analytics at lower cost.
We evaluate on $N=\datasetN$ Brazilian municipalities across $G=27$ states using a common evaluation protocol that includes:
...
Because these deliverables are regenerated across reporting cross-sections, the reduced datasets used for scalable computation
...
Kernel-based workloads (Gram matrices, kernel PCA, KRR) scale as $\mathcal{O}(N^2)$ and become a bottleneck across targets and reporting cross-sections \cite{scholkopf2002learning,engel2004krls,liu2010kaf}. Nyström
...
states and let $g_i\in\{1,\dots,G\}$ denote the state label of municipality $i$. For each state $g$, define the index set and its capacity as $I_g := \{ i\in\{1,\dots,N\} : g_i=g\}$, $n_g := |I_g|$.
A landmark set is a subset $S\subseteq\{1,\dots,N\}$ of fixed cardinality $|S|=k$. We also use the binary encoding $s\in\{0,1\}^N$ with 
$\sum_{i=1}^N s_i = k$, and the mapping $S=\{i:s_i=1\}$. State counts in a subset are $c_g(S) := |S\cap I_g|$, $c(S):=(c_1(S),\dots,c_G(S))$. We impose hard per-state bounds $\ell_g \le c_g(S)\le n_g$, where $g=1,\dots,G$ and where $\ell_g\ge 0$ can enforce minimum representation when required by the reporting interface. Hard group bounds of
...
\end{equation*}
and impose the feasibility requirement $D^{(w)}(S) \le \tau$, for a user-specified tolerance $\tau\ge 0$. KL divergence is a standard way to measure discrepancy between discrete
...
space so that evaluation results remain directly comparable across runs.
...
\subsection{Integrality Floors and Quota Paths (R0)}
...
\subsection{Proportionality as Feasibility (R6)}
...
\subsection{Population-Share Scaling (R1)}
...
Because R1 and R10 are each replicated across 5 seeds, we additionally report multi-seed stability diagnostics for the $k=300$ knee
solution. Table~\ref{tab:multi-seed-stats} provides mean/std/min/max summaries across seeds; a supplementary boxplot visualises the full distribution. These results support claims that the discovered
trade-offs are not an artefact of a single random initialisation or evaluation split. R10 replication further tests
...
\subsection{Downstream Utility and Robustness}
...
Table~\ref{tab:krr-multitask-k300} further reports target-wise RMSE for the representative R1 solution at $k=300$, reflecting the
...
\subsection{Ablations at Fixed Cardinality (R2--R7)}
...
Finally, R7 adds a latent-drift (SKL) third objective as a sensitivity check. Across all evaluation metrics in processed raw space, the tri-objective variant shows no consistent improvement over the bi-objective default, confirming that SKL does not provide non-redundant selection signal beyond MMD and Sinkhorn divergence.

\subsection{Baseline Comparisons (R10)}
...
\subsection{Space Transfer and Runtime (R8--R9)}
Runs R8 and R9 examine optimization-space transfer by optimizing in raw space and PCA space, respectively, while
evaluating all candidates under the common processed raw-space evaluation protocol.
...
Run R12 varies NSGA-II effort by sweeping six paired $(P,T)$ configurations---$(20,100)$, $(50,300)$, $(100,500)$, $(150,700)$, $(200,1000)$, and $(300,1500)$---at $k=300$. The effort sweep (detailed in supplementary material) shows that evaluation metrics plateau beyond $(P,T)=(200,1000)$, justifying the default settings; further increasing effort to $(300,1500)$ yields marginal improvements at approximately double the solver runtime.

Runs R13 and R14 vary the VAE latent dimension and the PCA dimension, respectively, over $\{8,16,32,64\}$ at $k=300$.
...
framework to temporal sequences so landmark sets track quarterly reporting cross-sections with bounded churn.
