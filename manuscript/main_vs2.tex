...
theoretically; empirical stability diagnostics are provided by the proxy stability experiment
...
Selection effort is controlled by NSGA-II population size and generation count; we report an effort sensitivity study quantifying the trade-off between selection runtime and downstream quality for a single configuration.
...
$w_i\equiv 1$ (quota planning and municipality-share quota experiments), while the weight-level analysis applies to the primary
population-share configuration and all experiments derived from it.
...
We study four composition-control regimes (see Table~\ref{tab:run-matrix} for the complete experimental matrix). Population-share proportionality ($w_i=\nu_i$, tolerance $\tau_{\mathrm{pop}}=0.02$) is the primary constraint and is used in the primary configuration, both objective ablations, the tri-objective ablation, both alternative representation-space experiments, and the effort and dimensionality sensitivity studies. Municipality-share quota mode ($w\equiv 1$, quota vector $c^\star(k)$ via Algorithm~\ref{alg:quota}, candidates restricted to $c(S)=c^\star(k)$) is used in the municipality-share quota experiment and for quota-matching baselines in the baseline comparison. The joint regime (quota mode plus population-share KL constraint) is used in the joint constraint experiment. Finally, the unconstrained ablation drops all proportionality constraints, retaining only exact-$k$ and per-state bounds, and serves to measure composition drift when proportionality is absent. A quota planning step computes $c^\star(k)$ and the feasibility floor $\KL_{\min}(k)$ for each $k\in\kGrid$.
...
Table~\ref{tab:run-matrix} summarizes the experimental matrix. The effort and dimensionality sensitivity studies expand into multiple concrete configurations but are grouped in the table by intent.
...
\begin{tabular}{p{0.14\textwidth}p{0.06\textwidth}p{0.10\textwidth}p{0.18\textwidth}p{0.12\textwidth}p{0.04\textwidth}p{0.28\textwidth}}
\toprule
Experiment & $k$ & Opt.\ space & Constraints & Objectives & Reps & Purpose \\
\midrule
Primary (VAE) & $\kGrid$ & VAE mean & population-share & MMD, Sinkhorn & 5 &
Cardinality analysis with full evaluation suite. \\
MMD-only abl.\ & 100 & VAE mean & population-share & MMD only & 5 &
Objective ablation (distribution matching only). \\
Sinkhorn-only abl.\ & 100 & VAE mean & population-share & Sinkhorn only & 5 &
Objective ablation (transport coverage only). \\
Muni-quota & 100 & VAE mean & municipality-share quota & MMD, Sinkhorn & 5 &
Quota mode (fixed histogram; within-state optimization). \\
Joint constr.\ & 100 & VAE mean & joint (quota + pop.-share) & MMD, Sinkhorn & 5 &
Strict feasibility under joint constraints. \\
Unconstrained abl.\ & 100 & VAE mean & none & MMD, Sinkhorn & 5 &
Constraint ablation (composition drift without proportionality). \\
Tri-objective abl.\ & 100 & VAE mean & population-share & MMD, Sink., LogDet & 5 &
Tri-objective ablation (Nystr\"{o}m log-determinant diversity). \\
Raw-space & $\kGrid$ & raw & population-share & MMD, Sinkhorn & 5 &
Optimization-space transfer: raw optimization, raw-space evaluation. \\
PCA-space & $\kGrid$ & PCA & population-share & MMD, Sinkhorn & 5 &
Optimization-space transfer: PCA optimization, raw-space evaluation. \\
Baselines & 100 & VAE & population-share quota & --- & 5 &
Eight baseline heuristics evaluated at $k{=}100$ with multi-seed replication. \\
Proxy stability & 100 & VAE/raw/PCA & diagnostics & MMD, Sinkhorn & 5 &
Surrogate stability and objective--metric alignment. \\
Effort sensitivity & 100 & VAE mean & population-share & MMD, Sinkhorn & 5 &
Six paired $(P,T)$ configurations from $(20,100)$ to $(300,1500)$. \\
VAE dim.\ sensitivity & 100 & VAE mean & population-share & MMD, Sinkhorn & 5 &
VAE dimensionality variation: $p\in\{8,16,32,64\}$. \\
PCA dim.\ sensitivity & 100 & PCA & population-share & MMD, Sinkhorn & 5 &
PCA dimensionality variation: $p\in\{8,16,32,64\}$. \\
\bottomrule
\end{tabular}
\end{table*}
...
\caption{Unconstrained exact-$k$ selection (R6, $k=100$): municipality-share drift versus Nyström approximation error.
...
\midrule
30  & 0.0743 & 0.2004 & 5.73 & 0.09 & 13.35 & 3.58 \\
50  & 0.0569 & 0.1402 & 5.47 & 0.08 & 13.05 & 3.43 \\
100  & 0.0432 & 0.0874 & 5.18 & 0.08 & 11.82 & 3.30 \\
200  & 0.0377 & 0.0668 & 5.03 & 0.07 & 11.96 & 3.22 \\
300  & 0.0302 & 0.0415 & 4.87 & 0.08 & 11.65 & 3.35 \\
400  & 0.0261 & 0.0297 & 4.70 & 0.08 & 11.19 & 3.15 \\
500  & 0.0234 & 0.0224 & 4.69 & 0.08 & 12.10 & 3.24 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
R1 & $e_{\mathrm{Nys}}$ & 0.0302 & 0.0007 & 0.0294 & 0.0315 \\
R1 & $e_{\mathrm{kPCA}}$ & 0.0415 & 0.0027 & 0.0396 & 0.0469 \\
R1 & RMSE 4G & 4.87 & 0.4883 & 4.39 & 5.78 \\
R1 & RMSE 5G & 0.0751 & 0.0623 & 0.0129 & 0.1551 \\
\bottomrule
\end{tabular}
\end{table}
...
Runs R2--R3 and R7 isolate the effect of objective choice at $k=100$. Figure~\ref{fig:objective_ablation_k300} projects
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{generated/figures/objective_ablation_bars_k100.pdf}
\caption{Objective ablation at $k=100$: feasible non-dominated sets for bi-objective (R1), MMD-only (R2), and
Sinkhorn-only (R3) selection projected into downstream metric space.}
\label{fig:objective_ablation_k300}
\end{figure}

Run R7 adds a third objective (Nystr\"{o}m log-determinant diversity). The tri-objective formulation yields no
consistent improvement over the bi-objective default, indicating that the additional objective does not contribute
...
at $k=100$. The unconstrained regime can improve operator fidelity while permitting nontrivial
geographic drift, consistent with the motivating analysis in Figure~\ref{fig:geo_ablation}.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{generated/figures/constraint_comparison_bars_k100.pdf}
\caption{Constraint regime comparison at $k=100$: feasible non-dominated sets for population-share (R1), quota mode (R4),
...
Run R10 benchmarks constrained multi-objective selection against a baseline suite at
$k=100$ with five-seed replication. All eight baseline methods---uniform sampling,
...
$c^\star_{\text{pop}}(k)$ quota vector as the NSGA-II search. Evaluation uses 453
representative downstream metrics spanning six families.
...
\begin{table}[t]
\centering
\caption{Grand mean rank across 453 downstream metrics ($k=100$, five replicates).
...
\midrule
1 & Herding (SKH)        & 4.30 & 4.0 & 42 & 20 \\
2 & Kernel Thinning (SKT)& 4.53 & 5.0 & 34 & 21 \\
3 & \textbf{NSGA-II knee}& 4.58 & 5.0 & 36 & 24 \\
4 & KKN (SKKN)           & 4.73 & 5.0 & 32 & 23 \\
5 & Uniform (SU)         & 4.77 & 5.0 & 33 & 26 \\
6 & Ridge Lev.\ (SRLS)   & 4.90 & 5.0 & 31 & 28 \\
7 & $k$-Means (SKM)      & 5.15 & 5.0 & 27 & 32 \\
8 & Farthest-First (SFF) & 5.96 & 6.0 & 21 & 48 \\
9 & $k$-DPP (SDPP)       & 6.08 & 7.0 & 18 & 52 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Geographic          &   9 & 1 & 1.00 \\
QoS Regression      & 110 & 4 & 4.49 \\
Supervised Reg.     & 117 & 3 & 4.61 \\
Classification      & 180 & 4 & 4.74 \\
KRR Regression      &  35 & 6 & 4.83 \\
Operator Fidelity   &   2 & 4 & 4.20 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
SU  (Uniform)       & 48.3 & 7.4 & 44.3 \\
SKM ($k$-Means)     & 51.2 & 7.6 & 41.2 \\
SKH (Herding)       & 43.4 & 7.8 & 48.8 \\
SFF (Far.-First)    & 61.9 & 7.3 & 30.9 \\
SRLS (Ridge Lev.)   & 51.4 & 7.5 & 41.1 \\
SDPP ($k$-DPP)      & 62.6 & 7.8 & 29.6 \\
SKT (Kernel Thin.)  & 44.7 & 7.5 & 47.8 \\
SKKN (KKN)          & 48.5 & 7.3 & 44.2 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Herding (SKH)            & 5.25 \\
Kernel Thinning (SKT)    & 5.51 \\
\textbf{R1 knee}         & 5.57 \\
R1 best-Sinkhorn         & 5.58 \\
R1 best-MMD              & 5.59 \\
KKN (SKKN)               & 5.77 \\
Uniform (SU)             & 5.81 \\
Ridge Lev.\ (SRLS)       & 6.00 \\
$k$-Means (SKM)          & 6.25 \\
Farthest-First (SFF)     & 7.26 \\
$k$-DPP (SDPP)           & 7.41 \\
\bottomrule
\end{tabular}
\end{table}
...
$k=100$; baselines above and to the right of the front are dominated on both displayed
objectives.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{generated/figures/baseline_comparison_grouped.pdf}
\caption{Baseline comparison at $k=100$: the R1 feasible non-dominated set (point cloud)
...
\midrule
VAE mean & \latentDim & 60\,339 & 83\,465 \\
Raw & $D$ & 7\,407 & 54\,779 \\
PCA & \latentDim & --- & 30\,026 \\
\bottomrule
\end{tabular}
\end{table}
