...
launch_job() {
    ...
    local CMD="cd ~/Coreset_Codes && source ~/Coreset_Codes/venv/bin/activate && export OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 OPENBLAS_NUM_THREADS=4 NUMEXPR_NUM_THREADS=4 && python3 scripts/bootstrap_reeval.py --run-id ${RUN_ID} --rep-id ${REP_ID} --n-bootstrap ${N_BOOT} --n-reg 5 --n-cls 5 --output-dir bootstrap_results/ 2>&1 | tee bootstrap_results/log_${RUN_ID}_rep${REP_FMT}.txt"
