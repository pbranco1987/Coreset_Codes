...
def run_bootstrap_evaluation(
    *,
    ...
    # NOTE: StandardScaler fitting is deferred to after splits are loaded
    # (Step 5) so we can fit on eval_train_idx only, avoiding data leakage.
    X_full = None  # placeholder — set after Step 5
    print(f"[bootstrap] X_full_raw: {X_full_raw.shape}")
    ...
    # Fit StandardScaler on training partition only to avoid data leakage,
    # then transform the full dataset (needed for Nyström features on any S).
    scaler = StandardScaler()
    scaler.fit(X_full_raw[eval_train_idx])
    X_full = scaler.transform(X_full_raw).astype(np.float64)
    print(f"[bootstrap] X_full: {X_full.shape} (scaler fit on {len(eval_train_idx)} train points)")
...
def main() -> None:
    """CLI entry point for bootstrap target re-evaluation.

    Parses command-line arguments, resolves data/cache paths, checks whether
    a prior (possibly partial) run exists for the same ``(run_id, rep_id)``,
    and either skips, resumes, or backs up and restarts depending on the
    detected state. Delegates the heavy lifting to
    :func:`run_bootstrap_evaluation` and persists the final CSV plus
    metadata sidecar under ``--output-dir``.

    Expects a valid experiment configuration at
    ``{experiments_dir}/{run_id}/rep{rep_id:02d}/config.json`` and a
    compatible replicate cache under ``{cache_dir}/rep{rep_id:02d}/``.

    This function is intended to be invoked from the command line; see
    ``python scripts/bootstrap_reeval.py --help`` for the full argument
    list.
    """
